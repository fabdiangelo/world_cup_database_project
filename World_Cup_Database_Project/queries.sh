...
echo "$($PSQL "SELECT name FROM games FULL JOIN teams ON winner_id=team_id WHERE year=2014 AND round='Eighth-Final' UNION ALL SELECT name FROM games FULL JOIN teams ON opponent_id=team_id WHERE year=2014 AND round='Eighth-Final' ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT name FROM games LEFT JOIN teams ON winner_id=team_id GROUP BY name ORDER BY name")"
